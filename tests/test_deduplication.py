...
""")
        ...
        assert dep_names.count("requests") > 1, "Should have duplicate 'requests' entries"
        ...
        assert package_names.count("requests") == 1, "Should have only one 'requests' entry in report"
        assert package_names.count("click") == 1, "Should have only one 'click' entry in report"
        assert package_names.count("numpy") == 1, "Should have only one 'numpy' entry in report"
        assert package_names.count("pytest") == 1, "Should have only one 'pytest' entry in report"
...
""")
        ...
        assert requests_pkg["version_spec"] == ">=2.25.0", "Should preserve more specific version"
...
""")
        ...
        assert pytest_pkg["dependency_type"] in ["runtime", "dev"], "Should have valid dependency type"
...
""")
        ...
        assert len(report["dependency_files"]) >= 2, "Should list multiple dependency files"
        assert any("requirements.txt" in f for f in report["dependency_files"]), "Should include requirements.txt"
        assert any("pyproject.toml" in f for f in report["dependency_files"]), "Should include pyproject.toml"
...
""")
        ...
        assert requests_pkg["version_spec"] == "==2.28.1", "Should preserve exact version over range"
...
""")
        ...
        requests_packages = [pkg for pkg in report["packages"] if pkg["name"].lower() == "requests"]
        assert len(requests_packages) == 1, "Should deduplicate case-insensitive package names"
...
""")
        ...
        requests_pkg = next(pkg for pkg in report["packages"] if pkg["name"] == "requests")
        assert requests_pkg["version_spec"] == ">=2.25.0", "Should preserve non-empty version spec"
...
""")
        ...
        assert numpy_pkg["version_spec"] == ">=1.22.0", "Should preserve highest minimum version"
